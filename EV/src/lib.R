...
topGOenrichment <- function(list_of_gene_names, allGenes, topN=100, pCutoff=0.001, type='all', output='input')
{
  ...
  pdf(paste(output,type,"topGO.pdf",sep="."), width=9, height=nrow(gt)/5)
    p = ggplot(gt, aes(x = Term, y = -log10(classicFisher), fill=ontology)) +
      ...
      ggtitle(paste("Top",topN,"enriched GO terms (p <",pCutoff,") in"), subtitle=type)
    print(p)
    for(i in c("BP","CC", "MF")){
      gt_sub = gt[gt['ontology']==i,]
      p = ggplot(gt_sub, aes(x = Term, y = -log10(classicFisher), fill=ontology)) +
      geom_bar(stat = "identity") +
      coord_flip() +
      theme_bw() + theme_classic() +
      xlab("GO terms") + ylab("-log10(Fisher's test P value)") +
      ggtitle(paste("Top",topN,"enriched GO-",i," terms (p <",pCutoff,") in"), subtitle=type)
      print(p)
    }
  dev.off()
}
...
ORA <- function(inputGenes, allGenes, topN=20, nCutoff=3, pCutoff=0.01, output='ORA.xls')
{
  ...
  gt=data.frame();
  for(i in c("c5.go.bp","c5.go.cc", "c5.go.mf")){
  ...
  write.table(gt, paste(output,"ORA_all.xls",sep="."), sep="\t", quote = F, col.names = NA, row.names = T)
  # filter
  gt = subset(gt, FDR <= pCutoff & n > nCutoff)
  gt = gt[with(gt, order(gene_set, -OR)), ]

  write.table(gt, paste(output,"ORA_pCutoff_nCutoff.xls",sep="."), sep="\t", quote = F, col.names = NA, row.names = T)
  gt0 = gt
  ...
  p = filter(gt, grepl("c5",gene_set)) %>% mutate(Term=factor(V1, unique(as.character(V1)))) %>%  
    ...
    theme_bw() + theme_classic() +
    xlab("GO terms") + ylab("-log10(Fisher's test P value)") + 
    ggtitle(paste("Top",topN,"enriched GO terms (p <",pCutoff,") in"), subtitle= "All")
  print(p)

  # for(i in c("bp","cc", "mf")){
  #     p = filter(gt, grepl(i,gene_set)) %>% mutate(Term=factor(V1, unique(as.character(V1)))) %>%
  #       ggplot(aes(x = Term, y = -log10(pvalue), fill=gene_set)) +
  #       geom_bar(stat = "identity") +
  #       coord_flip() +
  #       theme_bw() + theme_classic() +
  #       xlab("GO terms") + ylab("-log10(Fisher's test P value)") +
  #       ggtitle(paste("Top",topN,"enriched GO terms (p <",pCutoff,") in"), subtitle= i)
  #     print(p)
  #   }
  ...
  #   ggtitle(paste("Top",topN,"enriched pathways (p <",pCutoff,") in"), subtitle= "KEGG")
  # print(p)
  # 
  dev.off() 
